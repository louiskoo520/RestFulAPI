$(function(){
	...
	$("#addProblembtn").click(function(){
		...
		for(var i=1;i<=attrId;++i){
			...
			if(i == 1){ //默认处理者 ID,姓名
				item.selectText = $("#defaultDealer").val()+","+$("#defaultDealer").text();
			}
	...
	$("#addkindbtn").click(function(){
		if(attrId == 1){
			...
			'<input type="checkbox" id="require1">必填</div><div class="col fL">'+
			'<label class="tit">属性名：</label>'+
			'<input type="text" class="attrname" id="attrName1"/>'+
			...
			'<select id="defaultDealer" class="kindtype">'+
				'<option value="2">张世龙</option>'+
			'</select>'+
			'</div></div>';
			$(this).parent().parent().parent().append(tmp);
			++attrId;
			
			tmp = '<div class="kindlist clearfix">'+
			'<div class="col fL lh28"></div><div class="col fL lh28">'+
			'<input type="checkbox" id="require2">必填</div><div class="col fL">'+
			'<label class="tit">属性名：</label>'+
			'<input type="text" class="attrname" id="attrName2"/>'+
			'</div><div class="col fL colselect">'+
