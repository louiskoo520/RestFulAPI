function changeqs(val) {
	$(".qs").each(function(index, info){
		$(this).html(val);
	});
}
var startTime = "";
...
$(document).ready(function(){
	...
	function questionlist(state){
		...
		$.ajax({
			...
			dataType:"json",
			data:{state:state,pageIndex:pageIndex,start:startTime,end:endTime,key:keyWord,area:area,reportor:reportor},
			success:function(data){
				var topHtml = "<div class='searchss'>"+
										...
										"<div class='label label1'>上报人：</div><div class='label label3'><input type='text' class='reportor'></input></div>"+
...
//					for(var i=data.length-1;i>=0;i--){
	...
	$(document).on("click",".showdetail",function(){
		...
		var tqs = $(this).attr("qs");
		window.location.href = "questionstep.html?taskId="+taskId+"&qs="+tqs;
	});
	//根据当前页面和总页码展示页码。
