...
public class VRPackage {
	...
	public VRPackage(RendererPackage rendererPackage, double opacityWindow,
			double opacityLevel, vtkImageData imageData) {
		...
		this._volumeMapper.SetInputData(imageData);
	...
	void UpdateWWWL(double opacityWindow, double opacityLevel) {
		...
		this._colorFun.AddRGBSegment(opacityLevel - (0.5 * opacityWindow), 0.0,
				0.0, 0.0, opacityLevel + (0.5 * opacityWindow), 1.0, 1.0, 1.0);
		this._opacityFun.AddSegment(opacityLevel - (0.5 * opacityWindow), 0.0,
